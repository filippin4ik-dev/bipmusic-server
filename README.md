...
Каталог пустой, треки пропали — одна команда, дальше всё само:

```bash
cd /root/server && git fetch origin && git reset --hard origin/main && sh scripts/fix-db-path.sh
```

Находит базу, уехавшую мимо смонтированного тома, переносит её на правильный
путь, закрепляет абсолютный `DATABASE_URL`, пересобирает контейнеры и печатает,
сколько треков в базе. Повторный запуск безопасен. Разбор причины и ручные
варианты — [Пропали треки: каталог пустой](#пропали-треки-каталог-пустой).
