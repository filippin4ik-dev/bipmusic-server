...
```
Обычный текст — строки сами разойдутся по длительности трека,
подсветка будет идти за песней приблизительно.

[00:12.40] А с метками времени в начале строк
[00:16.90] подсветка точная,
[00:21.15] и тап по строке перематывает на неё.
```

Без меток клиент раскидывает строки пропорционально их длине с запасом на
вступление и проигрыш — ничего доразмечать не нужно, но на песне с длинным
проигрышем посередине текст будет уходить вперёд. Метки это лечат.
