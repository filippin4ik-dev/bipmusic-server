...
**Первым делом достань базу из контейнера — до любой пересборки.** Сборка удалит
старый контейнер вместе со слоем и базой, восстановить будет уже нечего:
...
Если файл скопировался и весит хотя бы сотни килобайт — треки вернутся. После
этого обновляй код: новый `DATABASE_URL` абсолютный, а entrypoint сам переносит
базу из `/app/prisma/data` на том, если найдёт её там.
